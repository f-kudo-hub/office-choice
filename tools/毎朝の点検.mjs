...
if (/読めませんでした/.test(検索の結果) || 検索の結果 === '') {
  // 2026-09-17〜 API で読む。読めない日は「数字が0」ではなく「読めていない」
  出す('- ✗ **Search Console を読めませんでした。**' + (検索の結果 ? '（' + 検索の結果.replace(/\s+/g, ' ').slice(0, 160) + '）' : ''))
  出す('  読み取り用アカウントが Search Console のユーザーに入っているかを確かめてください（00_検索の言葉.md に手順）。')
  要対応.push('Search Console を読めていません（点検できていません）')
} else {
  // 表示回数の合計と直近7日を、朝の1枚に1行で出す（まだ0に近い時期は、この行が唯一の手がかり）
  const 表示 = (検索の結果.match(/表示 (\d[\d,]*)回/) || [])[1]
  const 中身 = readFileSync('00_検索の言葉.md', 'utf8')
  const 週 = (中身.match(/直近7日の表示 \*\*(\d+)回\*\*（その前の7日 (\d+)回）/) || [])
  出す('- ○ 読めています（API・直近28日）。表示 **' + (表示 ?? '?') + '回**' + (週[1] != null ? '、直近7日 **' + 週[1] + '回**（前週 ' + 週[2] + '回）' : ''))
  if (/まだ Google の検索結果にほとんど出ていません/.test(中身)) {
    出す('  まだ検索結果にほとんど出ていません（サイト開始から ' + 送信から + '日）。**急ぎません。**増え始めたら「次に直す記事」が `00_検索の言葉.md` に出ます。')
  } else {
    出す('  **次に直す記事は `00_検索の言葉.md`** にあります。')
  }
  ...
  try {
    ...
    try {
      ...
      if (未公開.length) {
        ...
        出す('  📄 **手順は `note公開の手順.md` にあります**（出す10本・出さない1本・つまずく場所）。')
        出す('  🔗 https://note.com/notes を開くと下書きの一覧が出ます（「インポート」もこの画面です）。')
        出す('  ⚠ 先にChromeの拡張機能で **editor.note.com を許可**してください。')
        出す('  　公開ボタンは別ドメインにあり、許可が無いと画面が見えないまま押すことになります。')
        出す('  ⚠ **押す前に、本文の数字・日付・リンクが残っているか目で見てください。**')
        出す('  　（2026-08-31 の貼り付けでは、半角の数字と英字とURLが全部消えていました）')
        if (未公開.length >= 5) {
          要対応.push(
