  ...
  properties: {
    ...
    products: {
      ...
      items: {
        ...
        properties: {
          name: { type: 'string', description: 'えらぶ対象の分類名。特定の型番・製品名は書かない（在庫切れや改定で嘘になるため）。例: 業務用マイクロカットシュレッダー／クラウド勤怠管理システム（打刻方式が選べるもの）' },
          why: { type: 'string', description: 'どんな会社に向くか。2〜3文' },
          amazon_keyword: { type: 'string', description: 'Amazonで買える**物**のときだけ、検索窓に入れる日本語を3〜6語で書く。勤怠管理・電子契約・法人携帯・助成金など、Amazonで買えない**サービスや制度**のときは必ず空文字にする。迷ったら空文字。' },
...
/**
 * お題の指定（--お題=...）。
 * ふだんは題材をAIが自分で選びますが、**出口（提携先）はあるのに記事が無い**分野を
 * 埋めたいときは、こちらから指定できます（2026-09-02）。
 * 例：EMEAO!のLED照明工事は確定率66.66%で提携済みなのに、受け皿の記事がありませんでした。
 */
...
⚠ **あなた自身の体験として書いてはいけません。**この記事はAIが下書きしており、
そのことはページの末尾に明記されています。そこで本文が「私が過去に」「20年間で」
「経験上」と語ると、**同じページの中で嘘をついていることになります。**
実務の話は、体験談ではなく**一般に起きること**として書いてください。
...
${お題
  ? `**今回の題材は指定されています：${お題}**
この題材で1本書いてください。上の題名と重ならない切り口にしてください。`
...
4. 実務でしか出てこない話を必ず1つ以上入れる。
   ...
   ⚠ ただし**自分の体験として書かない。**「私が」「経験上」「見てきた」ではなく、
   「よくあるのは」「ここで詰まります」「確かめるのは次の点です」と書く。
   中身は同じで、嘘だけが消える。
5. 読者が最後に「で、どうすればいいか」がわかる状態で終える。
6. 敬体（です・ます）。1文は短く。専門用語には必ず言い換えを添える。
7. 見出しだけ読んでも筋が通るようにする。
8. **物とサービスを混ぜない。**その記事が「Amazonで買える物」の話なら amazon_keyword を入れる。
   勤怠管理・電子契約・法人携帯・複合機・電力・採用媒体・助成金など、**申し込む形のもの**は空文字にする。
   サービスの記事では、値段より「解約条件・初期費用・移行の手間・誰が運用するか」を厚く書く。
   継続契約は、入るときより**やめるときに詰まる。**そこを書けるのが決裁した人の強み。
`.trim()
...
async function 一本書く(何本目) {
  ...
  // ── 体験を装った1文が残っていないか ───────────────────────────
  //
  // ルールに書いただけでは止まらない。**現に止まらなかった。**
  // 2026-09-13、公開ずみ22本のうち6本に「私が過去に一度止めた案件は」
  // 「20年間で何度か入れ替えをして」といった一人称の体験が入っていた。
  // ページの末尾には「この記事はAIが下書きしています」と書いてある。
  // **同じページの中で食い違っていた。**
  //
  // これは失敗として現れない。記事は書け、組み立ても通り、公開もされる。
  // 読んだ人が気づいたときには、信用のほうが先に消えている。
  // だから書き出す前に、機械で読んで止める。
  const 体験を装う言い方 = /私[はがも]|わたし[はがも]|20年間|経験上|経験から言う|過去に見てき|担当してきた|やってきました|立ち会ってき/
  const 全文 = [記事.lead, ...(記事.sections ?? []).map(x => x.body)].join('\n')
  const 見つかった = 全文.match(体験を装う言い方)
  if (見つかった) {
    const 前後 = 全文.slice(Math.max(0, 見つかった.index - 40), 見つかった.index + 60)
    throw new Error(`体験を装った書き方が入っています（「${見つかった[0]}」）。…${前後}…`)
  }
