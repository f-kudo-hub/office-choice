 ...
 * 複数の語で引いて id で重ねる。
 */
// 2026-09-07：4語→12語→23語に増やした。**推測ではなく、実際に引いて数えた結果**で選んでいる。
//   4語のとき156件 → 12語で359件 → 23語で約390件。
//   ⚠ **ここが天井です。**この日、55語を実際に引いて確かめた。
//   「支援」「補助」「助成」「雇用」「脱炭素」「賃上げ」「ものづくり」「事業再構築」
//   「事業承継」「展示会」「省力化」「北海道」「大阪」「福岡」「農業」「介護」「保育」
//   「商店街」「個人事業主」ほか多数は、**200件近く返るのに新規は0件**だった。
//   つまり jGrants の「受付中」そのものが約390件しかない。**語を足しても500件にはならない。**
//   1,000ページは、件数ではなく**地域×用途の組み合わせページ**で作る（設計書 2026-09-07 参照）。
//   ⚠ **語を足すたびに、必ず引いて新規件数を数えること。**増やしただけで増えたと思わない。
const 検索語 = [
  '補助金',   // 168件（新規168）
  '助成金',   // 101件（新規94）
  '奨励金',   //  38件（新規35）
...
/**
 * ⚠ **日本時間で数えること。**`new Date().toISOString()` はUTCの日付を返す。
 * 更新は毎朝6時（日本時間）＝UTCでは前日の21時台なので、そのまま使うと
 * **毎日かならず前日の日付**が入り、サイトも説明文も1日古く見えていた（2026-09-09に判明）。
 * 締切までの残り日数もここを基準に数えるため、1日ぶん短く出ていた。
 */
const 今日 = 日付(new Date(Date.now() + 9 * 60 * 60 * 1000).toISOString())
